psytkReadData = function( surveyOrDir , psytkAllowRepeatExp = TRUE , verbose = TRUE ){
    ...
    setAnswers             = matrix( nrow = n , ncol=dim(psytkSetQuestions)[1] )
    colnames(setAnswers)   = psytkSetQuestions[,1]
    setScores              = setAnswers

    version                = rep( NA , n ) # PsyToolkitversion used
    test                   = rep( F , n )  # if survey was run as test, by default assume it is false
    startDate              = rep( NA , n ) 
    startTime              = rep( NA , n )
    ...
    if ( verbose  ){ catn("Now reading questionnaire data") }
        ...
        for ( l in 1:length(tmp) ){ # new in 1.0.4
            ...
            if ( words[1] == "l:" ){
                ...
                if ( words[2] %in% surveyobject$overview[,1] ){
                    ...
                    if ( verbose  ){ catn("Found label",currentLabel) }
                    ...
                    if ( l < length(tmp) ){
                        ...
                        if ( nextline == "t: set" ){ ## in this case, we need to add the variable to overview and to setAnswers
                            surveyobject$overview = rbind( surveyobject$overview , c( words[2] , "set" , 1 , "from experiment") ) ## add at bottom of overview the new set question
                            ...
                            currentLabel = words[2]
                            if ( verbose  ){ catn("Add label",currentLabel,"from experiments var out set") }
                        }else{
            ...
            if ( words[1] == "a:" & includeItem ){
                ...
                ## CHECK QUESTION #######################################
                ## for check questions it is empty (F) or not (T)
            ...
            if ( words[1] == "s:" & includeItem ){
                ...
                if ( currentType == "set" ){
                    setScores[ i , currentLabel ] = as.numeric( currentAnswer )
                }
                if ( currentType == "radio" ){
    ...
    ## needed for: scaleScores, radioScores, dropScores, setScores
    ##
    ...
    tmpmask = is.na(setScores)   & !is.na(setAnswers)   ; setScores[ tmpmask ]   = setAnswers[ tmpmask ]
        ...
        if ( length( projectIds ) > 0 ){
            for ( tmpId in projectIds ){
                ...
                if ( file.exists( file.path( psytkSurveyDir , expectedName ))){
                    if(verbose(catn("Now reading CloudResearch demographic data file: ",file.path( psytkSurveyDir , expectedName ))))
                    ...
                    if ( verbose ){
                        catn("No demographics file(s) found. Given that you use CloudResearch, you could put it here in your data folder.")
                    }
                }
            }
            if ( !is.null( tmpCRData ) ){
    ...
    if ( all( c("PROLIFIC_PID","STUDY_ID","SESSION_ID") %in% names(cgi) ) ){
        ...
        if ( verbose  ){
            catn("Data were collected with Prolific, now looking for Prolific demographic data.")
            catn("If you don't have these, you can download them and put in your data folder here on your local computer.")
        }
        ...
        if ( length( projectIds ) > 0 ){
            ...
            for ( tmpId in projectIds ){
                ...
                if ( file.exists( file.path( psytkSurveyDir , expectedName ))){
                    if(verbose(catn("Now reading Prolific demographic data file: ",file.path( psytkSurveyDir , expectedName ))))
                    ...
                    if(verbose){
                        catn("No demographics file(s) found. Given that you use Prolific, you could put it here in your data folder.")
                        catn("The file should start with prolific_demographic_export_ or prolific_export_")
                        catn("Maybe you have not downloaded it, don't need it, etc.")
                    }
                }
            }
            if ( !is.null( tmpPRData ) ){
    ...
    if ( verbose ){ hr(); catn("Now reading experiment data") ; hr() }

    for ( expname in psytkExperiments ){
        
        if( verbose ){ catn( "Now reading data for experiment",expname ) ; hr() }
        ...
        for ( i in 1:n ){
            ...
            if ( length(tmpFiles)>0 ){
                ...
                if ( verbose ) { catn(tmpWarning) }
                warnings = c( warnings , tmpWarning )
            }

            if ( length(tmpFiles)==1 | ( length(tmpFiles)>1 & psytkAllowRepeatExp ) ){
                if ( verbose ){ catn("Now read in file: ",tmpFilename) }
            ...
            if ( length( tmpFiles ) > 1 ){
                if ( psytkAllowRepeatExp ){
                    if ( verbose ) {
                        catn("Participant:",ID[i],expname,"more than 1 file for same experiment, but you allowed with psytkAllowRepeatExp" )
                    }
                }else{
                    if ( verbose ) {
                        catn("Participant:",ID[i],expname,"more than 1 file for same experiment, but not used (psytkAllowRepeatExp is FALSE)" )
                    }
                }
    ...
    if ( verbose ) { catn( "All questionnaire and experiment data are read in" )}

    if ( !is.null(warnings) ){
        hr()
        catn("There were a number of warnings in reading in PsyToolkit data (separate from regular R warnings):")
